...
* 금리시나리오가 수익률 곡선에 회귀하는 속도를 결정하는 모수 , 평균 회귀 계수(mean reversion coefficient) 금리가 평균 수준으로 회귀할 때 속도를 결정함. 알파가 클수록 평균수준에 빠르게 회귀. max (산출값, 0.0001)&#x20;
...
* 수익률곡선이 복원 가능하도록 조정하는 역할을 하는 함수 ?????
